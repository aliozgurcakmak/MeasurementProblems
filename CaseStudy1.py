...
def time_weighted_average(df):

    df["weight"] = 1 / (df["day_diff"] + 1)
    weighted_sum = (df["overall"] * df["weight"]).sum()
    total_weights = df["weight"].sum()
    weighted_average = weighted_sum / total_weights

    return weighted_average

weighted_avg_score = time_weighted_average(df)
# 4.6986738747386
...
top_20_reviews = sorted_df.head(20)["reviewText"]
